...
public:
    BinaryNode(const T &); // Constructor
private:
    T data;
    int x;
...
private:
    ...
    void recursivePreOrder(const BinaryNode<T> *) const;
    void recursiveInOrder(const BinaryNode<T> *) const;
    void recursivePostOrder(const BinaryNode<T> *) const;
    int recursiveCountNodes(const BinaryNode<T> *) const;
    int recursiveCountLeafNodes(const BinaryNode<T> *) const;
    int recursiveComputeHeightOfTree(const BinaryNode<T> *) const;
    void recursiveDeleteNodes(const BinaryNode<T> *);
    void recursiveDraw(BinaryNode<T> *node);
    BinaryNode<T>* getLeftmostNode(BinaryNode<T> *node) const;
    int getNodeLevel(BinaryNode<T> *node);
    int getPxLocOfLeftTree(const BinaryNode<T> *node);
    int getPxLocOfAncestor(const BinaryNode<T> *node);
    void resetNodePosition(BinaryNode<T> *node);
    bool recursiveDeleteAtLocation(BinaryNode<T> *node, int x, int y);
};

// Node constructor
template<typename T>
BinaryNode<T>::BinaryNode(const T &info) :
...
template<typename T>
bool BinarySearchTree<T>::recursiveDeleteAtLocation(BinaryNode<T> *node, int x, int y)
{
    if (node == 0)
        return false;
...
bool BinarySearchTree<T>::insert(const T &item)
{
    ...
    BinaryNode<T> *trailCurrentNode = root;
...
bool BinarySearchTree<T>::deleteItem(T item)
{
    ...
    if (currentNode->leftChild == 0 && currentNode->rightChild != 0)
    {
        ...
        if (trailCurrentNode->data < item)
        {
            BinaryNode<T> *tempPtr = trailCurrentNode->rightChild;
        ...
        {
            BinaryNode<T> *tempPtr = trailCurrentNode->leftChild;
...
// private
template<typename T>
void BinarySearchTree<T>::recursivePreOrder(const BinaryNode<T> *node) const
{
    if (node == 0)
        return;
    std::cout << node->data << " ";
...
template<typename T>
void BinarySearchTree<T>::recursiveInOrder(const BinaryNode<T> *node) const
{
    if (node == 0)
        return;
...
template<typename T>
void BinarySearchTree<T>::recursivePostOrder(const BinaryNode<T> *node) const
{
    if (node == 0)
        return;
...
QString BinarySearchTree<T>::getPostOrderTraversal() const
{
    ...
    if (this->isEmpty())
        return QString("");
    QStack<BinaryNode<T>*> stack1;
    QStack<BinaryNode<T>*> stack2;
    QString traversal;
    BinaryNode<T> *root = this->root;
    stack1.push(root);
...
template<typename T>
int BinarySearchTree<T>::recursiveCountNodes(const BinaryNode<T> *node) const
{
    if (node == 0)
        return 0;
...
template<typename T>
int BinarySearchTree<T>::recursiveCountLeafNodes(const BinaryNode<T> *node) const
{
    if (node == 0)
        return 0;
...
template<typename T>
int BinarySearchTree<T>::recursiveComputeHeightOfTree(const BinaryNode<T> *node) const
{
...
template<typename T>
void BinarySearchTree<T>::recursiveDeleteNodes(const BinaryNode<T> *node)
{
    if (node == 0)
        return;
...
template<typename T>
void BinarySearchTree<T>::resetNodePosition(BinaryNode<T> *node)
{
    if (node == 0)
        return;
...
void BinarySearchTree<T>::draw(QPainter *painter, double &scale)
{
    ...
    BinaryNode<T> *leftmost = getLeftmostNode(root);
...
template<typename T>
BinaryNode<T>* BinarySearchTree<T>::getLeftmostNode(BinaryNode<T> *node) const
{
    if (node->leftChild == 0)
        return node;
...
template<typename T>
int BinarySearchTree<T>::getNodeLevel(BinaryNode<T> *node)
{
    int level = 1;
...
template<typename T>
int BinarySearchTree<T>::getPxLocOfLeftTree(const BinaryNode<T> *node)
{
...
template<typename T>
int BinarySearchTree<T>::getPxLocOfAncestor(const BinaryNode<T> *node)
{
    // All ancestor's node->x will be 0 unless it has already been drawn -
    // find the ancestor who's x != 0
    BinaryNode<T> *currentNode = node->parent;
...
template<typename T>
void BinarySearchTree<T>::recursiveDraw(BinaryNode<T> *node)
{
    // Base case
    if (node == 0)
        return;
